...
class Utilities {
    ...
    static async isUserAuthenticated(req) {
        ...
        try {
            ...
            const response = await globalVar.libs.jwt.verify(token, globalVar.saltJwt);
            const tokenVerified = await globalVar.models.UserSession.findOne({ 'token': token });
        
            if (tokenVerified) {
                return Utilities.answerOk(response, globalVar.errors.authenticated, 200);
            } else {
                return Utilities.answerError(tokenVerified, globalVar.errors.invalidToken, 401);
            }
        } catch(error) {
            if (error.message === 'jwt expired') {
                // The token is already expired so I remove from UserSession table
                const deletedTokenResponse = await globalVar.models.UserSession.deleteOne({ token: req.headers['authorization'] });
                return Utilities.answerError(deletedTokenResponse, globalVar.errors.tokenExpired, 401);
            } else if (error.message === 'invalid signature' || error.message === 'jwt malformed' || error.message === 'invalid token') {
    ...
    static answerOk (data, message, statusCode) {
        if(!data) data = {};
        if(!message) message = '';
        return {...data, msg: message, statusCode: statusCode };
    }
    ...
    static answerError (error, message, statusCode) {
        ...
        return { error, msg: message, statusCode };
    }
    ...
    */
    ...
    static validateProducts (products, updateInventory) { 
        ...
        if(products.length === 0) {
            ...
            for (let i = 0; i < products.length; i++) {
                if (
                    updateInventory &&
                    (Object.keys(products[i]).indexOf('cost') === -1 || !products[i].cost) ||
                ...
                if (
                    ...
                    (products[i].productId.length !== 24 && products[i].productId.length !== 12) ||
                    !products[i].quantity || 
    ...
    static validateProductType (value) {
        if(value && (value === 'base' || value === 'sombra' || value === 'bolsa')) return true;
        return false;
    }
    ...
    static validatePaymentInformation (paymentInfo, paymentType) {
        ...
        switch (paymentType) {
            ...
            case 'zelle':
                ...
                return true;
            default: // El tipo de pago no es alguno de los comprobados anteriormente
                return false;
        }
    }
}
